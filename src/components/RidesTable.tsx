  ...
  if (!response.ok) {
    ...
    users.forEach(user => {
      ...
      if (user.driver !== null) {
        ...
        cell.innerHTML = `<h5 class="font-bold text-black dark:text-white">${user.first_name} ${ user.last_name }</h5>
        ...
        if ( user.driver.is_online ) {
          cell2.innerHTML = `<p class="inline-flex rounded-full bg-success bg-opacity-10 py-1 px-3 text-sm font-medium text-success">Online</p>`;
        } else {
          cell2.innerHTML = `<p class="inline-flex rounded-full bg-danger bg-opacity-10 py-1 px-3 text-sm font-medium text-danger">Offline</p>`;
        }

        const cell3 = row.insertCell();
        cell3.className = "border-b border-[#eee] py-5 px-4 dark:border-strokedark";
        if ( user.driver.is_verified ) {
          cell3.innerHTML = `<p class="inline-flex rounded-full bg-success bg-opacity-10 py-1 px-3 text-sm font-medium text-success">Verified</p>`;
        } else {
          cell3.innerHTML = `<p class="inline-flex rounded-full bg-danger bg-opacity-10 py-1 px-3 text-sm font-medium text-danger">Unverified</p>`;
        }
        ...
        cell.colSpan = 5;
        cell.className = "border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11";
        cell.innerHTML = `<p class="text-sm">There are currently no drivers, try to reload the page.</p>`;
      }
    });
  }
}
